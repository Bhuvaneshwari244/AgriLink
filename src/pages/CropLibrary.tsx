  ...
  if (selected) {
    return (
      <div className="container mx-auto px-4 py-6 max-w-3xl">
        ...
        <img src={resolveCropImage(selected.image, selected.name, selected.category)} alt={selected.name} className="w-full h-56 object-cover rounded-xl mb-4" referrerPolicy="no-referrer" crossOrigin="anonymous" onError={e => handleImageError(e, selected.name, selected.category)} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {filtered.map(crop => (
          <button key={crop.id} onClick={() => setSelected(crop)} className="glass-card overflow-hidden text-left hover:scale-[1.02] transition-transform">
            <img src={resolveCropImage(crop.image, crop.name, crop.category)} alt={crop.name} className="w-full h-32 object-cover" referrerPolicy="no-referrer" crossOrigin="anonymous" onError={e => handleImageError(e, crop.name, crop.category)} />
            <div className="p-3">
